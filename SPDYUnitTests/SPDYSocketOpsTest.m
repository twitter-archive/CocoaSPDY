...
- (void)testProxyReadOpInit
{
    ...
    STAssertEquals([op responseAsString].length, (NSUInteger)0, @"actual: %@", [op responseAsString]);
}
...
- (void)testProxyReadIncompleteTryParseFails
{
    ...
    STAssertTrue([op tryParseResponse], @"actual: %@", [op responseAsString]);
}
...
- (void)testProxyReadHasExtraData
{
    ...
    STAssertTrue([op tryParseResponse], @"response: %@", responseStr);
    STAssertTrue([op success], @"response: %@", responseStr);
    STAssertEquals(op->_bytesParsed, responseData.length - 9, nil);
}
